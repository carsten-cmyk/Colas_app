...
**Samleordre-visning unificeret (LÅST 2026-07-01):** I samleordre viser **Materiellevering** (Planlægning) og **Bekræftede biler** (Udførsel) nu de SAMME fulde visninger som enkelt-ordre, ikke længere en degraderet read-only sammenfatning.
- **Materiellevering:** interaktivt materiel-kort pr. child (kun anchor bærer materiel i mock → medfølgende børn viser "Ingen materiel planlagt"); materiel-send forbliver **ordre-global** (ÉN samlet knap). Per-child adresse-underoverskrift fjernet (redundant med venstre-rail).
- **Bekræftede biler:** viser **ÉN "Asfaltkørsel"-tabel + ÉN "Materiel kørsel"-tabel** med DAGENS fulde bil-/materiel-liste — IKKE pr. child. Bekræftede biler er pr. DAG: en multilæs-bil betjener flere børn men er stadig én fysisk bil, så der er altid netop én asfalt-tabel + én materiel-tabel (samme i enkelt-ordre og samleordre). Gate: tabellerne vises altid når dagens `vognmandBekraeftelse` findes (ingen `activeChild`-gate). *(Tidligere per-child-partitionering via `pre_fordeling` blev droppet 2026-07-01 — den gav duplikerede tabeller.)*

En audit (grep `isSamleordreMode` → 9 sektioner) bekræftede at kun disse 2 var divergente; de øvrige 7 samleordre-sektioner var allerede uniforme. Fane-baserede sektioner (Ordredetaljer/Dokumentation/Forundersøgelse/KS) har droppet "— {sted}"-suffiks i overskriften (fanen viser aktivt barn).
