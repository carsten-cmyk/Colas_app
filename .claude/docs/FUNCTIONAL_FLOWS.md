...
**Samleordre-visning unificeret (LÅST 2026-07-01):** I samleordre viser **Materiellevering** (Planlægning) og **Bekræftede biler** (Udførsel) nu de SAMME fulde/interaktive visninger som enkelt-ordre — **pr. child-ordre** (stakket under hinanden), ikke længere en degraderet read-only sammenfatning. Materiellevering: interaktivt materiel-kort pr. child (kun anchor bærer materiel i mock → medfølgende børn viser "Ingen materiel planlagt"); materiel-send forbliver **ordre-global** (ÉN samlet knap). Bekræftede biler: **fuld bil-tabel pr. child** (bil→child-tilskrivning via `vejesedler[].pre_fordeling[].ordre_id`; multilæs-bil kan optræde under flere børn; SMS-status delt pr. `regnr`) + materiel-transport-tabel **KUN under anchor**. En audit (grep `isSamleordreMode` → 9 sektioner) bekræftede at kun disse 2 var divergente; de øvrige 7 samleordre-sektioner var allerede uniforme (tabs + per-child data). Tabellerne vises ALTID i samleordre (ingen `activeChild`-gate).
